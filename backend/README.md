...
6. URL функции: `https://functions.yandexcloud.net/<FUNCTION_ID>?integration=raw`
7. **Привязать вебхук Telegram** — открыть в браузере (подставив токен и FUNCTION_ID, URL внутри — закодировать):
   ```
   https://api.telegram.org/bot<BOT_TOKEN>/setWebhook?url=https%3A%2F%2Ffunctions.yandexcloud.net%2F<FUNCTION_ID>%3Fintegration%3Draw
   ```
   Ответ `{"ok":true,...}` = вебхук установлен.

## После деплоя

Дать фронту URL функции (`https://functions.yandexcloud.net/<FUNCTION_ID>?integration=raw`) — он подставляется в `BACKEND_URL` в `js/app.js`, и заглушки заменяются на реальные вызовы (Сторис, сохранение на Android, оплата Stars).
